...
router.get('/my-points', authenticateToken, async (req, res) => {
  try {
    ...
    const monthlyPoints = await dbGet(
      `SELECT COALESCE(SUM(e.eco_points_awarded), 0) as points
...
router.get('/my-rank', authenticateToken, async (req, res) => {
  try {
    ...
    const totalUsers = await dbGet('SELECT COUNT(*) as count FROM users WHERE role IN (\'student\', \'volunteer\')');
...
router.post('/award', authenticateToken, authorizeRoles('organizer', 'admin'), async (req, res) => {
  try {
    ...
    // Get updated user
    const updatedUser = await dbGet('SELECT * FROM users WHERE id = ?', [user_id]);
...
router.get('/history/:userId', authenticateToken, async (req, res) => {
  try {
    ...
    if (userId != req.user.id && !['admin', 'organizer'].includes(req.user.role)) {
